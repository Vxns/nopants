...
exports.run = async (bot, message, args, ops) => {
    ...
    // Check for input
    if (!args[0]) return message.channel.send('Proper usage: .pollhn <Date>');
    
    // Create Embed
    const embed = new Discord.RichEmbed()
      .setColor("#ffffff") //To change color do .setcolor("#fffff")
      .setFooter (`Poll Created By ${message.author.username}`)
      .addField(`Requirements:`, `Itemlevel: 935+ / Traits: 70+ also to join Discord.`, true)
      ...
      .setTitle(`Dear Fam! The next Antours heroic Raid will be on ${args.join(' ')}`)
      .addField(`Progession:`, `❌ THIS IS NOT A PROGRESS RAID ❌ It´s more likely to help Guildies, for fun and a nice Time. (:`, true)
